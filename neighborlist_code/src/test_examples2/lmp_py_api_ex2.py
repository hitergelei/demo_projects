...
说明：总共近邻有64个，由于体系只有16个原子，故每个原子平均近邻个数是4个。
这意味着每个原子的邻居数量可能是不同的，但平均下来每个原子有 4 个邻居。总的邻居数量为 64，是因为所有原子的邻居数量加起来总共是 64。
(但是，对于每个中心原子而言，其对应的近邻原子不一定就是在截断半径内的原子，得计算和判断才知道)
"""
...
print("---------->判断粒子对之间的距离，如果小于cutoff 3.0，则输出")
